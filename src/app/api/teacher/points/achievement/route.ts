...
export async function GET() {
  return handler(async () => {
    ...
    const records = await store.getAchievementRecords(user.id);
...
export async function POST(req: Request) {
  return handler(async () => {
    ...
    if (!parsed.success) return bad("Invalid achievement point data.");
    const d = parsed.data;
    if (d.reason === "Custom" && !d.customReason.trim()) return bad("Provide a custom reason.");
    const rid = await store.addAchievementRecord({
      teacherId: user.id,
...
export async function DELETE(req: Request) {
  return handler(async () => {
    ...
    await store.deleteAchievementRecord(id, user.id);
    return ok({ ok: true });
  });
}
